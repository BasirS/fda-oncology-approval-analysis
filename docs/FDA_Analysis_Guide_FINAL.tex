...
I encountered a form of temporal confounding which fundamentally altered my conclusions. This pattern resembles Simpson's Paradox, where a lurking variable confounds the relationship between the primary variables, causing the apparent strength of an association to shift when the confounding variable is controlled \citep{box2005}. In my analysis, I initially observed a strong statistical interaction suggesting oncology drugs received a special benefit from Priority Review. However, I realized that oncology approvals were heavily concentrated in the modern era while non-oncology approvals were spread more evenly across history. Since the FDA has become faster overall in recent years, this temporal imbalance created a false signal in the pooled data. When I stratified the data by regulatory era, the apparent interaction vanished, which proved that the aggregate trend was an artifact of the data distribution rather than a genuine regulatory preference.
