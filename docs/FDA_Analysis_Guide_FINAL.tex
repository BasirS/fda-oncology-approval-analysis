...
\textbf{Primary Hypothesis (Main Effect):}

\textbf{$H_0$:} There is no difference in mean review times between oncology and non-oncology drugs, controlling for review designation and regulatory era. Formally: $\tau_{Oncology} = \tau_{Other}$.

\textbf{$H_a$:} Oncology drugs have different mean review times compared to non-oncology drugs. Formally: $\tau_{Oncology} \neq \tau_{Other}$.

\textbf{Secondary Hypothesis (Interaction Effect):}

\textbf{$H_0$:} There is no interaction between therapeutic area and review designation on log-transformed review times, controlling for regulatory era. Formally: $(\tau\beta)_{ij} = 0$ for all $i,j$.

\textbf{$H_a$:} The effect of Priority Review designation on review time differs between oncology and non-oncology drugs. Formally: $(\tau\beta)_{ij} \neq 0$ for at least one pair $i,j$.
...
The analysis revealed no statistically significant interaction between therapeutic area and review designation (F=2.43, p=0.119), indicating that Priority Review provides a consistent benefit across both oncology and non-oncology drugs. However, I discovered temporal confounding that manifested in a pattern similar to Simpson's Paradox when examining the main effect of therapeutic area across regulatory eras.
...
I utilized the FDA's official review classification as a critical covariate. I categorized applications as either ``Priority Review'' or ``Standard Review.'' This variable measures the FDA's intent; a Priority Review designation indicates that the agency determined the drug offered a significant improvement in safety or effectiveness \citep{fda2024}. I included this because any analysis of review speed that ignores Priority status would be confounded, as oncology drugs are more likely to receive this designation due to the severity of the condition they treat.
...
However, my FDA dataset is severely unbalanced. As shown in the exploratory data analysis, I have 739 ``Other'' drugs but only 299 ``Oncology'' drugs. Furthermore, the cell counts are lopsided: I have 224 Oncology-Priority drugs but only 75 Oncology-Standard drugs. Montgomery warns that ``the orthogonality property of main effects and interactions present in balanced data does not carry over to the unbalanced case'' \citep{montgomery2017}. Because the design is unbalanced, the factors are correlated. This creates a dilemma: variance that is shared between two factors (like Therapeutic Area and Regulatory Era) could mathematically be assigned to either one.
...
I chose Type III Sums of Squares because of the specific ``temporal clustering'' I identified in the dataset. My exploratory data analysis showed that Oncology approvals are heavily concentrated in the ``Post-FDASIA'' era (165 drugs) compared to the ``Pre-PDUFA'' era (25 drugs). Since the FDA has generally become faster over time, there is a strong correlation between being an oncology drug and being a modern (faster) approval.
...
\begin{figure}[h!]
    ...
    \caption{\fontsize{10}{12}\selectfont Cell means with 95\% confidence intervals by therapeutic area and review type. The bars show the arithmetic mean review time in days for each of the four experimental cells in the factorial design. Error bars represent 95\% confidence intervals calculated from the standard errors. Oncology Priority Review drugs showed the shortest mean review time (300 days), while non-oncology Standard Review drugs exhibited the longest (792 days).}
...
\begin{figure}[h!]
    ...
    \caption{\fontsize{10}{12}\selectfont Temporal trends in approval times across regulatory eras demonstrated systematic changes in review duration over time. The trend lines show the mean review time in days for each group across the four regulatory eras. The secular decline from Pre-PDUFA (970 days) to Post-FDASIA (384 days) illustrated the transformative impact of user fee legislation on FDA review efficiency. This baseline shift justified the use of regulatory era as a blocking factor in the ANOVA models.}
...
I calculated eta-squared using the formula:
\begin{equation}
\eta^2 = \frac{SS_{effect}}{SS_{total}}
\end{equation}

where $SS_{effect}$ represents the sum of squares for the model term of interest and $SS_{total}$ represents the total sum of squares in the model.

The regulatory era blocking factor accounted for the largest share of variance ($\eta^2 = 0.191$), confirming that temporal trends dominate the variability in FDA approval times. The review designation main effect explained 5.6\% of total variance which demonstrates a substantial impact of the Priority Review mechanism. The therapeutic area main effect explained 1.3\% of variance which indicates a modest but statistically significant difference between oncology and non-oncology drugs. The interaction term explained less than 0.2\% of variance which reinforces the conclusion that any differential benefit of Priority Review by therapeutic area is negligible in practical terms.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{../visualizations/fig5_effect_sizes_barchart_alt.png}
    \caption{\fontsize{10}{12}\selectfont Effect sizes (eta-squared) for main effects and interaction in the two-way ANOVA model. The bars represent the proportion of total variance in log review times explained by each model term. Regulatory era dominated with $\eta^2 = 0.191$ (19.1\%) which validated the blocking strategy. Review designation showed a substantial effect ($\eta^2 = 0.056$) while therapeutic area was modest ($\eta^2 = 0.013$) and the interaction was negligible ($\eta^2 = 0.002$).}
    \label{fig:effect_sizes}
\end{figure}

\FloatBarrier

The small effect size of therapeutic area ($\eta^2 = 0.013$) prompted investigation into whether specific regulatory mechanisms could explain this modest but significant difference.
...
To ensure the validity of these findings, I performed several robustness checks to address the uncertainty in the classification of some drugs. I re-ran the core models using a high-confidence subset of the data (n = 659), excluding records where the therapeutic area classification was based solely on orphan designation or proxy variables. The results showed that the Review Designation and Regulatory Era effects remained highly significant while the interaction term remained non-significant across all sensitivity specifications. (Note: The Pre-PDUFA era was excluded from this sensitivity analysis due to insufficient high-confidence data in that period, reducing the regulatory era degrees of freedom from 3 to 2.) This consistency across different subsets of the data strengthens the conclusion that the observed patterns are robust and not artifacts of classification errors or distributional anomalies.
...
\begin{figure}[h!]
    ...
    \caption{\fontsize{10}{12}\selectfont Direct comparison of ANOVA results between the primary model (n = 1038) and a sensitivity analysis where all `Uncertain' drugs were reclassified as `Other' (n = 1335). The plot shows that under this conservative assumption, the Therapeutic Area main effect weakens (F=1.83), while the core finding that Priority Review operates equitably remains consistent.}
...
\subsection{Temporal Confounding and the Importance of Blocking}

I uncovered temporal confounding during the era-stratified analysis which fundamentally changed the interpretation of the therapeutic area main effect. When I compared the naive baseline model (raw scale, unblocked) to the final adjusted model (log scale, era-blocked), the F-statistic for therapeutic area decreased from F=21.22 ($p < 0.001$) to F=18.70 ($p < 0.001$), indicating that part of the apparent oncology speed advantage was confounded with temporal trends rather than reflecting a pure therapeutic area effect.
...
\bottomrule
\end{tabular}
\caption{Table B.1: Comparison of F-statistics between primary analysis and high-confidence subset.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Table B.2: Permutation test validation of interaction effect.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Table C.1: ANOVA table for pooled analysis on raw-scale without era blocking.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Table C.2: ANOVA table for era-stratified model on log-scale (Type III SS).}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Table C.3: ANCOVA table with expedited program covariates. Note: Review Designation, Regulatory Era, and Interaction terms were included in the model but are not listed in this summary table. The full model included 10 degrees of freedom: Intercept (1) + Therapeutic Area (1) + Review Designation (1) + Regulatory Era (3) + Interaction (1) + Accelerated Approval (1) + Fast Track (1) + Orphan Drug (1).}
\end{table}
